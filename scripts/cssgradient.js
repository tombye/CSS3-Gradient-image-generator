...
self.prototype = {
	toWebkitGradient: function() {
		...
		if(!self.regex.keyword.test(this.direction)) {
			throw new Error('The direction is an angle that can‰Ûªt be converted.');
		}
		...
		for(var i=0; i<this.stops.length; i++) {
			...
			if(position === null && i > 0 && i < this.stops.length - 1) {
				...
				throw new Error('The position of the ' + stop.color + ' color stop is not a percentage. Can‰Ûªt convert.')
			}
